...
def fetch_lfn():
    try:
        ...
        run_tftp("test.128k", 32)
        run_tftp("test.4m", 128)
        run_tftp("test.4m", 256)
        run_tftp("test.4m", 512)
        tc_clean("tftp", "veth0")
        tc_clean("tftpd", "veth1")
...
create_random_file("/tmp/tftpd/test.64k", 1024*64)
...
create_random_file("/tmp/tftpd/test.4m", 1024*1024*4)
